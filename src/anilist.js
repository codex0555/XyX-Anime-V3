...
anilist.get('/net/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({ name, jname, rating, type, year, runt, storyline, genre, vido, vidox, eptoat, total });
        console.log(viewm);
    } catch (error) {
...
anilist.get('/watc/:id/:r/:t', async (req, res) => {
    try {
        ...
        res.status(200).json({ gw, watcid, watcr });
    } catch (error) {
